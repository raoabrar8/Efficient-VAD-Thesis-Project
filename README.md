...
The model achieves the following performance on the UCF-Crime dataset:

- **AUC-ROC**: ~0.85-0.90 (varies by configuration)
- **Accuracy**: ~80-85%
- **Precision**: ~75-80%
- **Recall**: ~80-85%
- **F1-Score**: ~78-82%

*Note: Actual results depend on training configuration and dataset split.*
...
3. **Score Distribution**: Normal vs. Abnormal score histograms
4. **Training Curves**: Loss and entropy over epochs
5. **t-SNE Visualization**: Feature space embedding
6. **Frame-level Anomaly Timeline**: Temporal anomaly detection
